...
# 공공 데이터 활용
## [1] 공공 데이터 포털
- 공공 데이터 : 공공 기관이 만들어 내는 공적인 자료나 정보
- 파일 데이터 : 오픈 API, 시각화 등 다양한 방식으로 제공
...
- 4단계 : 승인받은 오픈 API키 확인
- 5단계 : 승인받은 오픈 API 확인
- 6단계 : 오픈 API 활용(R 스크립트)

### 실시간 버스 위치 확인하기

## [3] 웹 스크래핑
- 웹 브라우저 화면에 표시되는 HTML 문서에서 사용자가 지정하거나 필요한 정보만 추출하여 가공하고 저장하며 사용자에게 제공하는 기술
 - 텍스트 마이닝에도 이용되고 있음
...
## [2] 상관관계
- x축 변수의 값이 증가하면 y축 변수의 값이 비례해서 증가하거나 반대로 x축 변수의 값이 증가하면 y축 변수의 값이 비례해서 감소하는 경우 두 변수는 상관관계에 있다고 말함
- 상관계수 : 두 변수가 어느 정도나 강한 상관관계인지 수치로 나타낸 지표
(1) 음의 상관관계
- 상관관계가 (-)인경우 -> X,Y가 반비례
- 상관계수가 -1에 가까울수록 강한 음의 상관관계에 있음
- 산점도상의 점들이 직선에 가까워짐
(2) 양의 상관관계
- 상관계수가 (+)인 경우 -> X,Y가 비례
- 상관계수가 1에 가까울수록 X,Y는 상관성이 강해짐
...
```
# (1) 데이터 확인
st <- data.frame(state.x77)         # 매트릭스를 데이터 프레임으로 변환
head(st)
...
### 2단계 데이터 수집
- 문제를 해결하기 위해 필요한 데이터가 무엇인지 파악하고, 이렇나 데이터들을 수집하는 과정

### 3단계 데이터 정제 및 전처리
- 수집된 데이터를 분석이 가능한 형태로 정돈하는 과정

### 4단계 데이터 탐색
- 이 단계부터가 사실상 데이터 분석에 해당
- 가벼운 데이터 분석
...
### 6단계 결과 보고
- 데이터와 분석과 해석이 마무리 되면 내용을 정리하여 보고서 형태로 작성
- 처음에 했던 문제 정의에 대해 문제를 해결하는데 도움이 되는 내용을 요약하는 것 

## [2] 탐색적 분석의 이해
- 전체 데이터 분석 단계에서 4번째에 해당하는 과정 => 수집된 데이터의 내용을 이해하기 위한 기초 분석 단계
- 수집 주제 : 통계에서 변수라는 용어로 사용
- 수집 주제가 하나인 1차원 데이터 = 단일변수 데이터
- 여러 개의 수집 주제에 대한 데이터를 하나로 모아 놓은 2차원 데이터 = 다중변수 데이터
...
```
- (3) 막대그래프 작성 결과
<img src="https://user-images.githubusercontent.com/100751115/236200819-72b8bbdb-00ea-43a5-8b97-86ca890b0a95.png">

- (4) 원그래프 작성 결과
<img src="https://user-images.githubusercontent.com/100751115/236201757-517d5e9e-55b3-4233-843e-0dc3290c9ca3.png">
